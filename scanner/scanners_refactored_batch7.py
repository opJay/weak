"""
Batch 7: 비즈니스 로직 및 설계 취약점 스캐너 (리팩토링)
OWASP Top 10 2025 A06 (Insecure Design) & A09 (Logging & Monitoring) 대응

7개 스캐너:
1. PriceManipulationScanner - 가격/수량 조작 탐지
2. RaceConditionScanner - 동시성 취약점
3. WorkflowBypassScanner - 워크플로우 우회
4. AccountEnumerationScanner - 계정 열거
5. ResourceExhaustionScanner - 리소스 소진
6. LoggingMonitoringScanner - 로깅/모니터링
7. BusinessLogicAnomalyScanner - 비즈니스 로직 이상
"""
...
class PriceManipulationScanner(BaseScanner):
        ...
        for param_name in params.keys():
            if any(price_word in param_name.lower() for price_word in self.PRICE_PARAMS):
                self.vulnerabilities.append({
                    ...
                    'severity': 'high',
                    'description': f'URL에 가격 관련 파라미터({param_name})가 노출되어 조작 가능성이 있습니다.',
                    'recommendation': '가격은 서버 측에서만 결정하고, 클라이언트 입력을 신뢰하지 마세요.'
                })

    def _check_price_forms(self) -> None:
        """폼에서 숨겨진 가격 필드 검사"""
            ...
            for input_tag in form.find_all('input'):
                ...
                if input_type == 'hidden' and any(p in input_name for p in self.PRICE_PARAMS):
                    self.vulnerabilities.append({
                        ...
                        'severity': 'high',
                        'description': f'숨겨진 가격 필드({input_name})가 발견되어 클라이언트에서 변조 가능합니다.',
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            self.vulnerabilities.append({
                                ...
                                'severity': 'critical',
                                'description': f'파라미터 {param_name}에 음수 값({test_value})이 허용됩니다.',
    ...
    def _calculate_severity(self) -> str:
        ...
        for level, value in severity_map.items():
