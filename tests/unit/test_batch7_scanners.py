...
class TestRaceConditionScanner:
    ...
    def test_true_positive_concurrent_success(self):
        ...
        # When
        scanner = RaceConditionScanner(url=url, http_client=mock_client)
        result = scanner.scan()

        # Then
        assert result['total'] > 0
        assert result['has_race_condition'] is True
        assert any('No Concurrency Control' in v['type'] for v in result['vulnerabilities'])

    @pytest.mark.unit
    def test_true_positive_state_changing_forms(self):
        """TP: 상태 변경 폼 탐지"""
        # Given
        html_content = '''
        ...
        # When
        scanner = RaceConditionScanner(html_content=html_content)
        result = scanner.scan()

        # Then
        assert result['total'] > 0
        assert any('State Changing Forms' in v['type'] for v in result['vulnerabilities'])

    @pytest.mark.unit
    def test_true_negative_no_forms(self):
        """TN: 상태 변경 폼 없음"""
        # Given
        html_content = '<p>Static content only</p>'
    ...
    @pytest.mark.unit
    def test_edge_case_concurrent_failure(self):
        """Edge Case: 동시 요청 실패"""
        # Given
        ...
        # Then
        assert result['total'] == 0 or result['severity'] == 'low'
